...
export async function getUsersFromDB(page: number, limit: number, sort: { field?: string, order?: 'asc' | 'desc' }, search_query: string): Promise<FetchUsersPayload> {
    const db = await openDB(DATABASE_NAME, DATABASE_VERSION);
    const tx = db.transaction(STORE_NAME, 'readonly');
    const store = tx.objectStore(STORE_NAME);
    const allUsers = await store.getAll();

    let filteredUsers = allUsers.filter(user => user.name.toLowerCase().includes(search_query.toLowerCase()));
    if (sort.order) {
